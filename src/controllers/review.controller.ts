...
const REVIEW_SERVICE_URL = 'http://localhost:8080/api';

// Get reviews by depositeeId
export const getReviewByDepositeeId = async (req: Request, res: Response) => {
   try {
      const response = await axios.get(
         `${REVIEW_SERVICE_URL}/reviews/depositee/${req.params.depositeeId}`
      );
...
export const deleteReview = async (req: Request, res: Response) => {
   try {
      await axios.delete(`${REVIEW_SERVICE_URL}/reviews/${req.params.id}`);
